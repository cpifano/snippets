...
(function ($) {
	...
	loadYoumax = function() {	
		...
		$('#youmax').append('<div id="youmax-encloser"><iframe id="youmax-video" width="'+(youmaxWidgetWidth-2)+'" height="'+(youmaxWidgetWidth/youmax_global_options.youtubeVideoAspectRatio)+'" src="" frameborder="0" allowfullscreen></iframe><div id="youmax-video-list-div"></div><div id="youmax-load-more-div">CARGAR MAS</div></div>');
	...
	showPlaylists = function(response,loadMoreFlag) {
		...
		for(var i=0; i<playlistArray.length; i++) {
			...
			if((i+youmax_global_options.youmaxItemCount-zeroPlaylistCompensation)%youmaxColumns!=0)
				$('#youmax-video-list-div').append('<div class="youmax-video-tnail-box" style="width:'+((100/youmaxColumns)-4)+'%;" id="'+playListId+'"><div class="youmax-video-tnail" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader( src=\''+playlistThumbnail+'\', sizingMethod=\'scale\'); background-image:url(\''+playlistThumbnail+'\')"><div class="youmax-playlist-sidebar" id="youmax-playlist-sidebar-'+playListId+'"><span class="youmax-playlist-video-count"><b>'+playlistSize+'</b><br/>VIDEOS</span></div></div><span class="youmax-video-list-title">'+playlistTitle+'</span><br/><span class="youmax-video-list-views">'+getDateDiff(playlistUploaded)+' atrás</span></div>');
			else
	...
	showUploads = function(response,playlistTitle,loadMoreFlag) {
		...
		for(var i=0; i<uploadsArray.length; i++) {
			...
			if((i+youmax_global_options.youmaxItemCount)%youmaxColumns!=0)
				$('#youmax-video-list-div').append('<div class="youmax-video-tnail-box" style="width:'+((100/youmaxColumns)-4)+'%;" id="'+videoId+'"><div class="youmax-video-tnail" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader( src=\''+videoThumbnail+'\', sizingMethod=\'scale\'); background-image:url(\''+videoThumbnail+'\')"><div class="youmax-duration"></div></div><span class="youmax-video-list-title">'+videoTitle+'</span><br/><span class="youmax-video-list-views">'+getDateDiff(videoUploaded)+' atrás</span></div>');
			else
	...
	displayVideoStats = function(response) {
		...
		for(var i=0; i<videoArray.length; i++) {
			...
			$videoThumbnail.find('.youmax-video-list-views').prepend(videoViewCount+' reproducciones | ');
	...
	resetLoadMoreButton = function() {
		...
		$youmaxLoadMoreDiv.text('CARGAR MAS');
	},
		...
		*/
		...
		$('#youmax-load-more-div').click(function(){
			...
			if(null!=nextPageToken && nextPageToken!="undefined" && nextPageToken!="") {
				...
				$youmaxLoadMoreDiv.html('Todo listo');
			}
		...
		if(youTubeChannelURL!=null) {
			...
			if(s!=-1) {
				...
				if(s!=-1) {
					...
					alert("No se pudo encontrar el canal..");
				}
			}
		}
